...
preprocessed_dir = "../data/preprocessed"  # New preprocessed directory
os.makedirs(preprocessed_dir, exist_ok=True)

IMG_SIZE = 128

# Function to preprocess images (runs once before training)
def preprocess_and_save_images():
    for data_dir in data_dirs:
        ...
        for class_name in sorted(os.listdir(data_dir)):
            ...
            output_class_dir = os.path.join(preprocessed_dir, f"{language}_{class_name}")
            os.makedirs(output_class_dir, exist_ok=True)
            
            # Apply random sampling for dgs dataset
            if language == 'dgs':
                all_files = [f for f in os.listdir(class_path) if f != ".DS_Store"]
                sampled_files = random.sample(all_files, min(100, len(all_files)))  # Sample 100 or less
            else:
                sampled_files = [f for f in os.listdir(class_path) if f != ".DS_Store"]
            
            for file in sampled_files:
                file_path = os.path.join(class_path, file)
                img = cv2.imread(file_path)
                padding = 60 if language == 'lse' else 30
                processed_img = preprocess_image(img, padding=padding)

                # Only save the image if the hand was detected
                if processed_img is not None:
                    cv2.imwrite(os.path.join(output_class_dir, file), processed_img)
...
def prepare_combined_dataset():
    ...
    for i, (class_path, class_name) in enumerate(all_classes):
        ...
        if 'dgs' in class_path and num_images > 100:
            image_files = random.sample([f for f in os.listdir(class_path) if f != '.DS_Store'], 100)
        else:
